"""Standalone comparison demo server.

This module powers the local comparison UI and is not part of the packaged
proxy/runtime API under ``uncommon_route``.
"""
...
def compare(q: Query):
    ...
    return CompareResponse(
        uncommon_route=RouterResult(
            tier=ur_tier,
            model=ur_model,
            confidence=round(ur.confidence, 3),
            cost_per_1k=round(_cost_per_1k(ur_model) * 1000, 4),
            color=TIER_COLORS.get(ur_tier, "#888"),
            latency_us=round(ur_us, 0),
            signals=ur.signals[:5],
        ),
        clawrouter=RouterResult(
            tier=cr_tier,
            model=cr_model,
            confidence=round(cr_conf, 3),
            cost_per_1k=round(_cost_per_1k(cr_model) * 1000, 4),
            color=TIER_COLORS.get(cr_tier, "#888"),
            latency_us=round(cr_us, 0),
            signals=[],
        ),
        claude_opus=RouterResult(
            tier=claude_tier,
            model="claude-opus-4.6 (judge)",
