...
SIMPLE_B5: list[dict] = [
    ...
    _c("What is the current status?", "SIMPLE", "factual-qa", "en",
       sys_prompt="You are monitoring a Kubernetes cluster with 50 microservices."),
    _c("Is the build passing?", "SIMPLE", "factual-qa", "en",
...
COMPLEX_B5: list[dict] = [
    # ── New domains ──
    _c("Design an IoT fleet management platform with device provisioning, OTA firmware updates, telemetry ingestion, anomaly detection, and remote configuration.", "COMPLEX", "system-design", "en"),
    _c("Build a multiplayer game server with authoritative physics, client-side prediction, lag compensation, matchmaking, and anti-cheat.", "COMPLEX", "system-design", "en"),
    _c("Design a bioinformatics pipeline for whole-genome sequencing: read alignment, variant calling, annotation, quality control, and clinical reporting.", "COMPLEX", "system-design", "en"),
    _c("Build a smart contract system for decentralized exchange with order matching, liquidity pools, slippage protection, and MEV prevention.", "COMPLEX", "complex-code", "en"),
    _c("Design an edge AI inference platform with model optimization, hardware-aware deployment, A/B testing, and federated model updates.", "COMPLEX", "architecture", "en"),

    # ── Adversarial: LOOKS medium (single sentence) but IS COMPLEX ──
    _c("Implement a Raft consensus library with leader election, log replication, snapshotting, and dynamic membership changes", "COMPLEX", "complex-code", "en"),
    _c("Build a WebRTC SFU with room management, bandwidth estimation, simulcast, and recording", "COMPLEX", "complex-code", "en"),

    # ── More non-English COMPLEX ──
    _c("设计一个物联网设备管理平台，包括设备注册、固件升级、遥测数据采集、异常检测和远程配置", "COMPLEX", "system-design", "zh"),
    _c("构建一个去中心化交易所的智能合约系统，包括订单匹配、流动性池、滑点保护和 MEV 防护", "COMPLEX", "complex-code", "zh"),
    _c("IoTフリート管理プラットフォームを設計してください。デバイスプロビジョニング、OTAアップデート、テレメトリ、異常検知を含めてください。", "COMPLEX", "system-design", "ja"),
    _c("IoT 디바이스 관리 플랫폼을 설계하세요. 디바이스 프로비저닝, OTA 업데이트, 텔레메트리 수집, 이상 탐지를 포함해야 합니다.", "COMPLEX", "system-design", "ko"),
    _c("صمم منصة إدارة أجهزة إنترنت الأشياء تشمل تسجيل الأجهزة وتحديث البرامج عن بعد وجمع البيانات واكتشاف الشذوذ.", "COMPLEX", "system-design", "ar"),
    _c("Спроектируй платформу для управления IoT-устройствами с провизионированием, OTA-обновлениями, сбором телеметрии и обнаружением аномалий.", "COMPLEX", "system-design", "ru"),
    _c("Projete uma plataforma de gerenciamento IoT com provisionamento, atualizações OTA, coleta de telemetria e detecção de anomalias.", "COMPLEX", "system-design", "pt"),
    _c("Diseña una plataforma de gestión IoT con aprovisionamiento de dispositivos, actualizaciones OTA, recopilación de telemetría y detección de anomalías.", "COMPLEX", "system-design", "es"),
    _c("Entwerfe eine IoT-Geräteverwaltungsplattform mit Provisionierung, OTA-Updates, Telemetrieerfassung und Anomalieerkennung.", "COMPLEX", "system-design", "de"),
    _c("Conçois une plateforme de gestion IoT avec provisionnement, mises à jour OTA, collecte de télémétrie et détection d'anomalies.", "COMPLEX", "system-design", "fr"),
]


REASONING_B5: list[dict] = [
    # ── New topics ──
    _c("Prove that the problem of determining whether a context-free grammar is ambiguous is undecidable. Use reduction from the Post correspondence problem.", "REASONING", "formal-proof", "en"),
    _c("Derive the information-theoretic lower bound for lossless data compression (Shannon's source coding theorem). Prove step by step.", "REASONING", "math-derivation", "en"),
    _c("Prove that the stable matching produced by the Gale-Shapley algorithm is optimal for the proposing side. Use contradiction.", "REASONING", "algorithm-proof", "en"),
    _c("In a repeated prisoner's dilemma with discount factor δ, derive the minimum δ for which cooperation is sustainable in a subgame-perfect equilibrium.", "REASONING", "game-theory", "en"),
    _c("Prove that the knapsack problem is NP-complete by reduction from subset sum. Show both directions.", "REASONING", "formal-proof", "en"),

    # ── Adversarial: LOOKS like explanation but IS REASONING ──
    _c("Why can't we solve the traveling salesman problem efficiently? Prove that TSP is NP-hard.", "REASONING", "formal-proof", "en"),
    _c("Explain why P ≠ NP is believed to be true. Give a formal argument using diagonalization.", "REASONING", "formal-proof", "en"),
    ...
    _c("Gale-Shapleyアルゴリズムが提案側にとって最適であることを証明してください。背理法を用いてください。", "REASONING", "algorithm-proof", "ja"),
    _c("게일-셰플리 알고리즘이 제안측에 최적임을 증명하세요. 귀류법을 사용하세요.", "REASONING", "algorithm-proof", "ko"),
    _c("أثبت أن مسألة الحقيبة NP-complete عن طريق التخفيض من مسألة مجموع الجزئي.", "REASONING", "formal-proof", "ar"),
    _c("Докажи, что задача о рюкзаке является NP-полной путём сведения от задачи о сумме подмножеств.", "REASONING", "formal-proof", "ru"),
    _c("Prove que o problema da mochila é NP-completo por redução do problema da soma de subconjuntos.", "REASONING", "formal-proof", "pt"),
    _c("Demuestra que el problema de la mochila es NP-completo mediante reducción desde el problema de la suma de subconjuntos.", "REASONING", "formal-proof", "es"),
    _c("Beweise, dass das Rucksackproblem NP-vollständig ist durch Reduktion vom Teilsummenproblem.", "REASONING", "formal-proof", "de"),
    _c("Démontre que le problème du sac à dos est NP-complet par réduction du problème de la somme de sous-ensembles.", "REASONING", "formal-proof", "fr"),
]
