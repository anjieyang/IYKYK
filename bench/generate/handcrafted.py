...
SIMPLE: list[dict] = [
    ...
    _c("I was just wondering if you could possibly tell me what the programming language Python is mainly used for?", "SIMPLE", "factual-qa", "en"),
    _c("Could you please explain to me in very basic terms what an API is and what it stands for?", "SIMPLE", "factual-qa", "en"),
    _c("I'm new to programming and I was curious about what exactly a variable is in programming languages?", "SIMPLE", "factual-qa", "en"),
...
MEDIUM: list[dict] = [
    ...
    _c("What's the best way to handle errors here?", "MEDIUM", "explanation", "en",
       sys_prompt="You are a senior Python developer reviewing code."),
    _c("How should I structure this component?", "MEDIUM", "explanation", "en",
...
COMPLEX: list[dict] = [
    ...
    _c("Design a distributed rate limiter that works across multiple microservice instances using Redis. Include the algorithm, data structures, and handle edge cases like clock skew.", "COMPLEX", "system-design", "en"),
    _c("Design a URL shortener service that handles 10M daily requests, with analytics, custom aliases, and expiration support.", "COMPLEX", "system-design", "en"),
    _c("Design a notification system supporting push, email, SMS, and in-app channels with priority queues, rate limiting, and user preferences.", "COMPLEX", "system-design", "en"),
    _c("Design a real-time chat system with message persistence, read receipts, typing indicators, file sharing, and group conversations.", "COMPLEX", "system-design", "en"),
    _c("Design a file storage service like Dropbox with versioning, sharing, deduplication, and conflict resolution for offline edits.", "COMPLEX", "system-design", "en"),

    # ── English complex code ──
    _c("Implement a full CRUD REST API with authentication, input validation, error handling, and database migrations using FastAPI and SQLAlchemy", "COMPLEX", "complex-code", "en"),
    _c("Write a comprehensive test suite for a payment processing module covering happy paths, edge cases, race conditions, and failure recovery", "COMPLEX", "complex-code", "en"),
    _c("Build a real-time collaborative text editor with operational transformation, conflict resolution, and offline support", "COMPLEX", "complex-code", "en"),
    _c("Design and implement a custom database query optimizer that handles JOIN reordering, index selection, and cost-based plan selection", "COMPLEX", "complex-code", "en"),
    _c("Implement a B+ tree from scratch in Rust with concurrent access, page splitting, and range queries", "COMPLEX", "complex-code", "en"),
    _c("Build a web crawler that respects robots.txt, handles rate limiting, deduplicates URLs, and supports distributed crawling", "COMPLEX", "complex-code", "en"),
    _c("Implement a full OAuth2 PKCE flow with refresh token rotation, token revocation, and device fingerprinting", "COMPLEX", "complex-code", "en"),
    _c("Build a task queue system with priority scheduling, dead letter handling, retry with backoff, and distributed locking", "COMPLEX", "complex-code", "en"),

    # ── English architecture ──
    _c("Refactor this monolithic application into microservices. Identify service boundaries, define API contracts, and design inter-service communication.", "COMPLEX", "architecture", "en"),
    _c("Architect a real-time event processing pipeline using Kafka with exactly-once semantics, dead letter queues, schema evolution, and multi-region failover", "COMPLEX", "architecture", "en"),
    _c("Design a multi-tenant SaaS platform with data isolation, billing integration, RBAC, and elastic scaling", "COMPLEX", "architecture", "en"),

    # ── English security ──
    _c("Analyze the security vulnerabilities in this authentication flow and propose fixes for CSRF, XSS, SQL injection, and session fixation", "COMPLEX", "security-analysis", "en"),
    _c("Perform a security audit of this API: check for broken authentication, IDOR, rate limiting gaps, and data exposure risks", "COMPLEX", "security-analysis", "en"),

    # ── English infrastructure ──
    _c("Create a Kubernetes deployment with auto-scaling, health checks, rolling updates, and a service mesh for a multi-tenant SaaS app", "COMPLEX", "infrastructure", "en"),
    _c("Set up a complete CI/CD pipeline with build, test, lint, security scan, staging deploy, canary release, and rollback automation", "COMPLEX", "infrastructure", "en"),

    # ── English ML pipeline ──
    _c("Design an end-to-end ML pipeline for fraud detection including feature engineering, model training, A/B testing, monitoring, and automated retraining on drift", "COMPLEX", "ml-pipeline", "en"),
    _c("Build a real-time recommendation system with collaborative filtering, content-based fallback, A/B testing, and cold start handling", "COMPLEX", "ml-pipeline", "en"),

    # ── English migration ──
    _c("Plan a zero-downtime migration from PostgreSQL to CockroachDB for 500M rows with schema changes, validation, and rollback strategy", "COMPLEX", "migration", "en"),
    _c("Migrate a monolithic Django app to a FastAPI microservices architecture with service discovery, shared auth, and data migration", "COMPLEX", "migration", "en"),

    # ── English performance ──
    _c("Optimize this React app: implement code splitting, virtualized lists, memoization, image lazy loading, and service worker caching", "COMPLEX", "performance", "en"),

    # ── English short-but-complex ──
    _c("Implement distributed consensus with Raft including leader election, log replication, and membership changes", "COMPLEX", "complex-code", "en"),
    _c("Build a type checker for a subset of TypeScript supporting generics, union types, and structural typing", "COMPLEX", "complex-code", "en"),

    # ── Chinese ──
    _c("设计一个支持百万级并发的分布式消息队列系统，包括消息持久化、消费者组、死信队列和监控告警", "COMPLEX", "system-design", "zh"),
    _c("实现一个完整的用户认证系统，包括 JWT、OAuth2、RBAC 权限控制、密码加密和审计日志", "COMPLEX", "complex-code", "zh"),
    _c("设计一个支持多租户的 SaaS 平台架构，包括数据隔离、计费系统、权限管理和弹性伸缩方案", "COMPLEX", "architecture", "zh"),
    ...
    # ── Russian ──
    _c("Спроектируй распределённую систему кэширования с инвалидацией, репликацией и шардированием данных", "COMPLEX", "system-design", "ru"),
    _c("Реализуй полноценный REST API с аутентификацией, валидацией, обработкой ошибок и миграциями базы данных", "COMPLEX", "complex-code", "ru"),

    # ── Japanese ──
    _c("認証、レート制限、サーキットブレーカー、分散トレーシングを含むAPIゲートウェイを設計してください", "COMPLEX", "architecture", "ja"),

    # ── Korean ──
    _c("분산 캐싱 시스템을 설계하세요. 캐시 무효화, 복제, 샤딩, 장애 복구를 포함해야 합니다.", "COMPLEX", "system-design", "ko"),

    # ── German ──
    _c("Entwerfe eine skalierbare Microservice-Architektur mit Service Discovery, Load Balancing, Circuit Breaking und verteiltem Tracing.", "COMPLEX", "architecture", "de"),

    # ── With system prompt ──
    _c("The current latency is 2 seconds per request. Bring it under 200ms.", "COMPLEX", "performance", "en",
...
REASONING: list[dict] = [
    ...
    _c("Prove that the halting problem is undecidable using a diagonalization argument. Show each step formally.", "REASONING", "formal-proof", "en"),
    _c("Prove by mathematical induction that the sum of the first n natural numbers equals n(n+1)/2", "REASONING", "formal-proof", "en"),
    ...
    _c("Prove that every continuous function on [a,b] is uniformly continuous. Use epsilon-delta.", "REASONING", "formal-proof", "en"),
    ...
    _c("Derive the time complexity of the Fibonacci function using the Master Theorem. Prove step by step.", "REASONING", "math-derivation", "en"),
    _c("Solve step by step: derive the probability of exactly 10 defects in 500 widgets with P(defect)=0.02 using Poisson", "REASONING", "math-derivation", "en"),
    _c("Derive Bayes' theorem from the definition of conditional probability. Show all steps.", "REASONING", "math-derivation", "en"),
    _c("Derive the closed-form solution for the recurrence T(n) = 2T(n/2) + n", "REASONING", "math-derivation", "en"),

    # ── English formal logic ──
    _c("Using formal logic, prove that if all men are mortal and Socrates is a man, then Socrates is mortal. Use first-order predicate logic.", "REASONING", "formal-logic", "en"),
    _c("Prove de Morgan's laws using truth tables and then using formal logical derivation", "REASONING", "formal-logic", "en"),

    # ── English algorithm proof ──
    _c("Prove the correctness of Dijkstra's algorithm using loop invariants. Show initialization, maintenance, and termination.", "REASONING", "algorithm-proof", "en"),
    _c("Prove that the greedy algorithm for fractional knapsack is optimal using the exchange argument", "REASONING", "algorithm-proof", "en"),
    _c("Prove that merge sort has O(n log n) time complexity in all cases using the recursion tree method", "REASONING", "algorithm-proof", "en"),

    # ── English game theory / logic puzzle ──
    _c("In a game where players alternately remove 1-3 stones from 15, prove the first player has a winning strategy via backward induction", "REASONING", "game-theory", "en"),
    _c("Prove that in the Monty Hall problem, switching gives 2/3 probability. Use formal probability theory.", "REASONING", "game-theory", "en"),
    _c("A farmer must cross a river with a wolf, goat, and cabbage. Prove the minimum crossings is 7 and show the unique solution.", "REASONING", "logic-puzzle", "en"),
    ...
    # ── Russian ──
    _c("Докажи по индукции, что сумма первых n нечётных чисел равна n². Покажи каждый шаг формально.", "REASONING", "formal-proof", "ru"),
    _c("Выведи формулу Байеса шаг за шагом. Докажи математически.", "REASONING", "math-derivation", "ru"),

    # ── Japanese ──
    _c("数学的帰納法を用いて 1+2+...+n = n(n+1)/2 を証明してください。各ステップを形式的に。", "REASONING", "formal-proof", "ja"),

    # ── Korean ──
    _c("수학적 귀납법으로 1+2+...+n = n(n+1)/2를 증명하세요. 각 단계를 형식적으로.", "REASONING", "formal-proof", "ko"),

    # ── German ──
    _c("Beweise durch vollständige Induktion, dass die Summe der ersten n natürlichen Zahlen n(n+1)/2 beträgt.", "REASONING", "formal-proof", "de"),

    # ── Spanish ──
    _c("Demuestra por inducción matemática que la suma de los primeros n números naturales es n(n+1)/2.", "REASONING", "formal-proof", "es"),

    # ── French ──
    _c("Démontre par récurrence que la somme des n premiers entiers naturels vaut n(n+1)/2.", "REASONING", "formal-proof", "fr"),
]
