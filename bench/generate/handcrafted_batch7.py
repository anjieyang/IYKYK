...
MEDIUM_B7: list[dict] = [
    ...
    _c("Check the error rate dashboard, find the spike, and correlate with recent deploys", "MEDIUM", "agentic-task", "en"),
...
COMPLEX_B7: list[dict] = [
    # ── New domains ──
    _c("Design a robotics control system with sensor fusion, path planning, obstacle avoidance, real-time control loops, and failure recovery.", "COMPLEX", "system-design", "en"),
    _c("Build a music production pipeline with audio ingestion, beat detection, stem separation, mixing automation, and format export.", "COMPLEX", "system-design", "en"),
    _c("Design a climate data analysis platform with satellite data ingestion, time-series processing, anomaly detection, visualization, and predictive modeling.", "COMPLEX", "system-design", "en"),
    _c("Design a hardware-in-the-loop testing platform with FPGA simulation, real-time data acquisition, fault injection, and automated test orchestration.", "COMPLEX", "system-design", "en"),
    _c("Build a real-time multiplayer physics engine with deterministic simulation, rollback netcode, client prediction, and spectator mode.", "COMPLEX", "complex-code", "en"),

    # ── More complex code ──
    _c("Implement a custom allocator for Rust with slab allocation, thread-local caching, and fragmentation mitigation.", "COMPLEX", "complex-code", "en"),
    _c("Build a SQL query engine with lexer, parser, query planner, join optimization, and execution engine.", "COMPLEX", "complex-code", "en"),
    _c("Implement a peer-to-peer file sharing protocol with DHT, NAT traversal, chunk verification, and bandwidth management.", "COMPLEX", "complex-code", "en"),
    _c("Build a real-time collaborative whiteboard with vector graphics, multi-cursor support, undo history, and conflict-free replication.", "COMPLEX", "complex-code", "en"),
    _c("Implement a custom TLS 1.3 handshake library with certificate validation, key exchange, and session resumption.", "COMPLEX", "complex-code", "en"),

    # ── More security / infra / migration ──
    _c("Design a supply chain security system for container images with SBOM generation, vulnerability scanning, image signing, and policy enforcement.", "COMPLEX", "security-analysis", "en"),
    _c("Set up a multi-cloud Kubernetes federation with unified service mesh, cross-cluster DNS, and centralized policy management.", "COMPLEX", "infrastructure", "en"),
    _c("Plan a migration from REST to gRPC with backward compatibility, protobuf schema management, and gradual traffic shifting.", "COMPLEX", "migration", "en"),

    # ── Performance / ML ──
    _c("Optimize a video transcoding pipeline for 4K content: GPU acceleration, parallel processing, adaptive bitrate, and storage optimization.", "COMPLEX", "performance", "en"),
    _c("Design an end-to-end NLP pipeline with data cleaning, tokenization, embedding training, fine-tuning, evaluation, and deployment.", "COMPLEX", "ml-pipeline", "en"),

    # ── Chinese COMPLEX new ──
    _c("设计一个机器人控制系统，包括传感器融合、路径规划、避障、实时控制回路和故障恢复", "COMPLEX", "system-design", "zh"),
    _c("实现一个 SQL 查询引擎，包括词法分析、语法分析、查询规划、连接优化和执行引擎", "COMPLEX", "complex-code", "zh"),
    _c("设计一个容器镜像供应链安全系统，包括 SBOM 生成、漏洞扫描、镜像签名和策略执行", "COMPLEX", "security-analysis", "zh"),
    _c("优化一个 4K 视频转码流水线：GPU 加速、并行处理、自适应码率和存储优化", "COMPLEX", "performance", "zh"),

    # ── Japanese COMPLEX new ──
    _c("ロボット制御システムを設計してください。センサー融合、経路計画、障害物回避、リアルタイム制御ループを含めてください。", "COMPLEX", "system-design", "ja"),
    _c("カスタムSQLクエリエンジンを実装してください。レキサー、パーサー、クエリプランナー、結合最適化を含めてください。", "COMPLEX", "complex-code", "ja"),

    # ── Korean COMPLEX new ──
    _c("로봇 제어 시스템을 설계하세요. 센서 퓨전, 경로 계획, 장애물 회피, 실시간 제어 루프를 포함해야 합니다.", "COMPLEX", "system-design", "ko"),
    _c("SQL 쿼리 엔진을 구현하세요. 렉서, 파서, 쿼리 플래너, 조인 최적화를 포함해야 합니다.", "COMPLEX", "complex-code", "ko"),

    # ── Arabic COMPLEX new ──
    _c("صمم نظام تحكم روبوتي يشمل دمج أجهزة الاستشعار وتخطيط المسار وتجنب العقبات وحلقات التحكم في الوقت الفعلي.", "COMPLEX", "system-design", "ar"),
    _c("صمم نظام أمان سلسلة التوريد لصور الحاويات يشمل إنشاء SBOM ومسح الثغرات وتوقيع الصور.", "COMPLEX", "security-analysis", "ar"),

    # ── Portuguese COMPLEX new ──
    _c("Projete um sistema de controle robótico com fusão de sensores, planejamento de caminho, desvio de obstáculos e loops de controle em tempo real.", "COMPLEX", "system-design", "pt"),
    _c("Implemente um mecanismo de consulta SQL com lexer, parser, planejador de consulta e otimização de join.", "COMPLEX", "complex-code", "pt"),

    # ── Russian COMPLEX new ──
    _c("Спроектируй систему управления роботами с объединением сенсоров, планированием маршрута, обходом препятствий и контурами управления в реальном времени.", "COMPLEX", "system-design", "ru"),
    _c("Реализуй SQL-движок с лексером, парсером, планировщиком запросов и оптимизацией соединений.", "COMPLEX", "complex-code", "ru"),

    # ── Spanish COMPLEX new ──
    _c("Diseña un sistema de control robótico con fusión de sensores, planificación de rutas, evasión de obstáculos y bucles de control en tiempo real.", "COMPLEX", "system-design", "es"),

    # ── German COMPLEX new ──
    _c("Entwerfe ein Robotersteuerungssystem mit Sensorfusion, Pfadplanung, Hinderniserkennung und Echtzeit-Steuerungsschleifen.", "COMPLEX", "system-design", "de"),

    # ── French COMPLEX new ──
    _c("Conçois un système de contrôle robotique avec fusion de capteurs, planification de chemin, évitement d'obstacles et boucles de contrôle temps réel.", "COMPLEX", "system-design", "fr"),

    # ── Hindi COMPLEX ──
    _c("एक रोबोटिक नियंत्रण प्रणाली डिज़ाइन करें जिसमें सेंसर फ्यूजन, पथ नियोजन, बाधा टालना और वास्तविक समय नियंत्रण शामिल हो।", "COMPLEX", "system-design", "hi"),

    # ── Turkish COMPLEX ──
    _c("Sensör füzyonu, yol planlama, engel kaçınma ve gerçek zamanlı kontrol döngülerini içeren bir robot kontrol sistemi tasarlayın.", "COMPLEX", "system-design", "tr"),

    # ── Vietnamese COMPLEX ──
    _c("Thiết kế một hệ thống điều khiển robot bao gồm hợp nhất cảm biến, lập kế hoạch đường đi, tránh chướng ngại vật và vòng lặp điều khiển thời gian thực.", "COMPLEX", "system-design", "vi"),

    # ── Polish COMPLEX ──
    _c("Zaprojektuj system sterowania robotem z fuzją sensorów, planowaniem ścieżki, omijaniem przeszkód i pętlami sterowania w czasie rzeczywistym.", "COMPLEX", "system-design", "pl"),
]
...
REASONING_B7: list[dict] = [
    # ── New proofs ──
    _c("Prove that the set of algebraic numbers is countable while the set of transcendental numbers is uncountable.", "REASONING", "formal-proof", "en"),
    _c("Prove the Bolzano-Weierstrass theorem: every bounded sequence in R has a convergent subsequence.", "REASONING", "formal-proof", "en"),
    _c("Prove that a language is regular if and only if it can be recognized by a finite automaton. Show both directions.", "REASONING", "formal-proof", "en"),
    _c("Prove that the chromatic number of a planar graph is at most 5 using the five-color theorem.", "REASONING", "formal-proof", "en"),
    _c("Prove that the space of continuous functions on [0,1] with the sup norm is complete (a Banach space).", "REASONING", "formal-proof", "en"),
    _c("Prove Zorn's lemma is equivalent to the axiom of choice. Show both directions.", "REASONING", "formal-proof", "en"),

    # ── New derivations ──
    _c("Derive the optimal step size for gradient descent on a quadratic function. Prove convergence rate.", "REASONING", "math-derivation", "en"),
    _c("Derive the VC dimension of linear classifiers in d dimensions. Prove it equals d+1.", "REASONING", "math-derivation", "en"),
    _c("Derive the expected depth of a randomly built binary search tree. Prove it's O(log n).", "REASONING", "math-derivation", "en"),
    _c("Derive the capacity of a binary symmetric channel with error probability p. Show the proof.", "REASONING", "math-derivation", "en"),

    # ── New algorithm proofs ──
    _c("Prove that A* search with an admissible heuristic finds the optimal path. Use induction.", "REASONING", "algorithm-proof", "en"),
    _c("Prove the amortized O(1) cost of dynamic array doubling using the potential method.", "REASONING", "algorithm-proof", "en"),
    _c("Prove that the union-find data structure with path compression and union by rank achieves O(α(n)) amortized.", "REASONING", "algorithm-proof", "en"),

    # ── More logic / game theory ──
    _c("Prove that satisfiability of propositional logic (SAT) is NP-complete using Cook's theorem.", "REASONING", "formal-logic", "en"),
    _c("In a Bayesian game with incomplete information, derive the optimal strategy for the informed player in a simple signaling game.", "REASONING", "game-theory", "en"),
    _c("Prove that in a congestion game, a pure Nash equilibrium always exists using a potential function.", "REASONING", "game-theory", "en"),
    ...
    _c("A*探索が許容可能なヒューリスティックで最適パスを見つけることを証明してください。", "REASONING", "algorithm-proof", "ja"),

    # ── Korean REASONING new ──
    _c("볼차노-바이어슈트라스 정리를 증명하세요: R의 유계 수열은 수렴하는 부분 수열을 가진다.", "REASONING", "formal-proof", "ko"),
    ...
    _c("Prove o teorema de Bolzano-Weierstrass: toda sequência limitada em R tem uma subsequência convergente.", "REASONING", "formal-proof", "pt"),
    _c("Derive o tamanho ótimo do passo para gradiente descendente em uma função quadrática.", "REASONING", "math-derivation", "pt"),

    # ── Russian REASONING new ──
    _c("Докажи теорему Больцано-Вейерштрасса: каждая ограниченная последовательность имеет сходящуюся подпоследовательность.", "REASONING", "formal-proof", "ru"),
    _c("Выведи оптимальный шаг градиентного спуска для квадратичной функции.", "REASONING", "math-derivation", "ru"),

    # ── Spanish REASONING new ──
    _c("Demuestra el teorema de Bolzano-Weierstrass: toda sucesión acotada en R tiene una subsucesión convergente.", "REASONING", "formal-proof", "es"),
    _c("Deriva el tamaño de paso óptimo para el descenso por gradiente en una función cuadrática.", "REASONING", "math-derivation", "es"),

    # ── German REASONING new ──
    _c("Beweise den Satz von Bolzano-Weierstraß: Jede beschränkte Folge in R hat eine konvergente Teilfolge.", "REASONING", "formal-proof", "de"),

    # ── French REASONING new ──
    _c("Démontre le théorème de Bolzano-Weierstrass : toute suite bornée dans R admet une sous-suite convergente.", "REASONING", "formal-proof", "fr"),

    # ── Hindi REASONING ──
    _c("बोल्ज़ानो-वीयरस्ट्रास प्रमेय सिद्ध करें: R में हर बाउंडेड अनुक्रम में एक अभिसारी उपअनुक्रम होता है।", "REASONING", "formal-proof", "hi"),

    # ── Turkish REASONING ──
    _c("Bolzano-Weierstrass teoremini kanıtlayın: R'de her sınırlı dizinin yakınsak bir alt dizisi vardır.", "REASONING", "formal-proof", "tr"),

    # ── Vietnamese REASONING ──
    _c("Chứng minh định lý Bolzano-Weierstrass: mọi dãy bị chặn trong R đều có dãy con hội tụ.", "REASONING", "formal-proof", "vi"),

    # ── Polish REASONING ──
    _c("Udowodnij twierdzenie Bolzano-Weierstrassa: każdy ciąg ograniczony w R ma podciąg zbieżny.", "REASONING", "formal-proof", "pl"),
]
