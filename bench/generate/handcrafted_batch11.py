...
MEDIUM_B11: list[dict] = [
    ...
    _c("Implement subnet calculation: given IP and CIDR, return network and broadcast addresses", "MEDIUM", "simple-code", "en"),
    ...
    _c("Write a function to find strongly connected components using Tarjan's algorithm", "MEDIUM", "simple-code", "en"),
...
COMPLEX_B11: list[dict] = [
    ...
    _c("Design a compiler frontend for a toy language with lexer, parser, AST, semantic analysis, and IR generation. Include error recovery and symbol table management.", "COMPLEX", "system-design", "en"),
    _c("Build a JIT compiler with bytecode interpretation, profiling, hot path detection, and native code generation. Include deoptimization support.", "COMPLEX", "system-design", "en"),
    _c("Design a multi-pass optimizer with constant propagation, dead code elimination, inlining, and register allocation. Include cost models for each pass.", "COMPLEX", "system-design", "en"),
    _c("Implement a compiler backend targeting a custom ISA with instruction selection, scheduling, and register allocation. Include peephole optimization.", "COMPLEX", "complex-code", "en"),

    # ── Operating systems multi-requirement ──
    _c("Design a microkernel with process management, IPC, virtual memory, and device drivers. Include scheduling policies, paging, and capability-based security.", "COMPLEX", "system-design", "en"),
    _c("Build a userspace threading library with stack management, context switching, synchronization primitives, and integration with the system scheduler.", "COMPLEX", "complex-code", "en"),
    _c("Design a file system with inodes, directory structure, journaling, and crash recovery. Include allocation strategies and caching.", "COMPLEX", "system-design", "en"),
    _c("Implement a memory manager with buddy allocation, slab allocator, and page reclaim. Include OOM handling and cgroup integration.", "COMPLEX", "complex-code", "en"),

    # ── Network protocols multi-requirement ──
    _c("Design a TCP stack with congestion control, flow control, retransmission, and connection management. Include handling of out-of-order packets and duplicate ACKs.", "COMPLEX", "system-design", "en"),
    _c("Build an HTTP/2 implementation with multiplexing, stream prioritization, header compression, and flow control. Include server push and connection migration.", "COMPLEX", "complex-code", "en"),
    _c("Design a BGP implementation with route reflection, policy filtering, and multipath. Include convergence optimization and route dampening.", "COMPLEX", "system-design", "en"),
    _c("Implement a QUIC stack with 0-RTT, connection migration, and multiplexing. Include TLS integration and loss detection.", "COMPLEX", "complex-code", "en"),

    # ── Database internals multi-requirement ──
    _c("Design a storage engine with WAL, buffer pool, B-tree indexes, and checkpointing. Include crash recovery, vacuum, and statistics collection.", "COMPLEX", "system-design", "en"),
    _c("Build a query executor with volcano model, join algorithms (nested loop, hash, merge), aggregation, and sort. Include cost-based plan selection.", "COMPLEX", "complex-code", "en"),
    _c("Design a replication system with synchronous and asynchronous modes, failover, and conflict resolution. Include split-brain prevention.", "COMPLEX", "system-design", "en"),
    _c("Implement an LSM-based storage engine with leveled compaction, bloom filters, and range queries. Include write amplification optimization.", "COMPLEX", "complex-code", "en"),

    # ── Distributed consensus multi-requirement ──
    _c("Implement a Raft consensus module with leader election, log replication, membership changes, and snapshotting. Include handling of network partitions.", "COMPLEX", "complex-code", "en"),
    _c("Design a distributed transaction coordinator with 2PC, 3PC fallback, and compensation. Include timeout handling and orphan resolution.", "COMPLEX", "system-design", "en"),
    _c("Build a replicated state machine with consensus, persistence, and linearizable reads. Include lease-based read optimization.", "COMPLEX", "complex-code", "en"),
    _c("Design a multi-datacenter replication system with conflict resolution, causal consistency, and failover. Include topology awareness.", "COMPLEX", "system-design", "en"),

    # ── Real-time systems multi-requirement ──
    _c("Design a real-time scheduler with EDF, rate monotonic, and mixed criticality support. Include admission control and overload handling.", "COMPLEX", "system-design", "en"),
    _c("Build an automotive ECU software stack with AUTOSAR, CAN communication, diagnostic services, and OTA updates. Include safety monitoring.", "COMPLEX", "system-design", "en"),
    _c("Design a hard real-time system with WCET analysis, priority inheritance, and deterministic memory allocation. Include certification support.", "COMPLEX", "system-design", "en"),
    _c("Implement a time-triggered architecture with slot-based scheduling, fault tolerance, and redundancy. Include clock synchronization.", "COMPLEX", "complex-code", "en"),

    # ── Signal processing / robotics multi-requirement ──
    _c("Design a real-time signal processing pipeline with FFT, filtering, feature extraction, and classification. Include latency bounds and buffering.", "COMPLEX", "system-design", "en"),
    _c("Build a robot motion planning system with inverse kinematics, collision detection, trajectory optimization, and real-time execution. Include joint limits.", "COMPLEX", "complex-code", "en"),
    _c("Design a sensor fusion system with Kalman filter, EKF for non-linear dynamics, and multi-sensor integration. Include outlier rejection.", "COMPLEX", "system-design", "en"),
    _c("Implement a computational geometry library with convex hull, Voronoi, Delaunay, and range queries. Include robustness for degenerate cases.", "COMPLEX", "complex-code", "en"),

    # ── Graph algorithms multi-requirement ──
    _c("Design a graph analytics platform with distributed storage, BFS/DFS/PageRank, and community detection. Include incremental updates.", "COMPLEX", "system-design", "en"),
    _c("Build a routing engine with shortest path, multi-criteria optimization, and dynamic updates. Include contraction hierarchies.", "COMPLEX", "complex-code", "en"),
    _c("Design a flow network solver with max-flow, min-cut, and multi-commodity flow. Include capacity scaling and push-relabel.", "COMPLEX", "system-design", "en"),
    _c("Implement a graph database with property graph model, Cypher-like query, and index support. Include transaction isolation.", "COMPLEX", "complex-code", "en"),

    # ── Type theory / category theory multi-requirement ──
    _c("Design a type checker for a language with Hindley-Milner, algebraic data types, and type classes. Include inference and error reporting.", "COMPLEX", "complex-code", "en"),
    _c("Build a proof assistant kernel with dependent types, tactics, and term normalization. Include metaprogramming support.", "COMPLEX", "complex-code", "en"),
    _c("Design a category-theoretic library with functors, monads, applicatives, and free monads. Include laws verification and documentation.", "COMPLEX", "system-design", "en"),
    _c("Implement a bidirectional type checker with subtyping and gradual typing. Include blame tracking.", "COMPLEX", "complex-code", "en"),

    # ── Non-English COMPLEX ──
    _c("设计一个编译器前端，包括词法分析、语法分析、AST、语义分析和 IR 生成。需要错误恢复和符号表管理。", "COMPLEX", "system-design", "zh"),
    _c("设计一个 TCP 协议栈，包括拥塞控制、流量控制、重传和连接管理。需要处理乱序包和重复 ACK。", "COMPLEX", "system-design", "zh"),
    _c("设计一个存储引擎，包括 WAL、缓冲池、B 树索引和检查点。需要崩溃恢复、vacuum 和统计收集。", "COMPLEX", "system-design", "zh"),
    _c("实现一个 Raft 共识模块，包括领导者选举、日志复制、成员变更和快照。需要处理网络分区。", "COMPLEX", "complex-code", "zh"),
    _c("设计一个实时调度器，支持 EDF、速率单调和混合关键级。需要准入控制和过载处理。", "COMPLEX", "system-design", "zh"),
    _c("设计一个传感器融合系统，包括卡尔曼滤波、非线性 EKF 和多传感器集成。需要异常值剔除。", "COMPLEX", "system-design", "zh"),

    _c("コンパイラフロントエンドを設計してください。レキサー、パーサー、AST、意味解析、IR生成を含め、エラー回復とシンボルテーブル管理を含めてください。", "COMPLEX", "system-design", "ja"),
    _c("TCPスタックを設計してください。輻輳制御、フロー制御、再送、接続管理を含め、順序不同パケットと重複ACKの処理を含めてください。", "COMPLEX", "system-design", "ja"),
    _c("Raftコンセンサスモジュールを実装してください。リーダー選出、ログ複製、メンバー変更、スナップショットを含め、ネットワーク分断の処理を含めてください。", "COMPLEX", "complex-code", "ja"),

    _c("컴파일러 프론트엔드를 설계하세요. 렉서, 파서, AST, 의미 분석, IR 생성, 오류 복구, 심볼 테이블 관리를 포함해야 합니다.", "COMPLEX", "system-design", "ko"),
    _c("TCP 스택을 설계하세요. 혼잡 제어, 흐름 제어, 재전송, 연결 관리, 순서 바뀐 패킷 및 중복 ACK 처리를 포함해야 합니다.", "COMPLEX", "system-design", "ko"),
    _c("Raft 합의 모듈을 구현하세요. 리더 선출, 로그 복제, 멤버십 변경, 스냅샷, 네트워크 분할 처리를 포함해야 합니다.", "COMPLEX", "complex-code", "ko"),

    _c("صمم واجهة مترجم تشمل المحلل المعجمي والنحوي وAST والتحليل الدلالي وتوليد IR. تضمن استعادة الأخطاء وإدارة جدول الرموز.", "COMPLEX", "system-design", "ar"),
    _c("صمم مكدس TCP مع التحكم في الازدحام والتدفق وإعادة الإرسال وإدارة الاتصال. تضمن معالجة الحزم غير المرتبة وACK المكررة.", "COMPLEX", "system-design", "ar"),

    _c("Спроектируй фронтенд компилятора: лексер, парсер, AST, семантический анализ, генерация IR. Включи восстановление после ошибок и таблицу символов.", "COMPLEX", "system-design", "ru"),
    _c("Спроектируй стек TCP с управлением перегрузкой, потоком, повторной передачей и соединениями. Включи обработку неупорядоченных пакетов.", "COMPLEX", "system-design", "ru"),

    _c("Diseña un frontend de compilador con lexer, parser, AST, análisis semántico y generación de IR. Incluye recuperación de errores y tabla de símbolos.", "COMPLEX", "system-design", "es"),
    _c("Diseña un stack TCP con control de congestión, flujo, retransmisión y gestión de conexiones. Incluye manejo de paquetes desordenados.", "COMPLEX", "system-design", "es"),

    _c("Entwerfe ein Compiler-Frontend mit Lexer, Parser, AST, semantischer Analyse und IR-Generierung. Inkl. Fehlerbehandlung und Symboltabelle.", "COMPLEX", "system-design", "de"),
    _c("Entwerfe einen TCP-Stack mit Congestion Control, Flow Control, Retransmission und Verbindungsverwaltung. Inkl. Behandlung von Out-of-Order-Paketen.", "COMPLEX", "system-design", "de"),

    _c("Conçois un frontend de compilateur avec lexer, parser, AST, analyse sémantique et génération IR. Inclure récupération d'erreurs et table des symboles.", "COMPLEX", "system-design", "fr"),
    _c("Conçois une pile TCP avec contrôle de congestion, flux, retransmission et gestion des connexions. Inclure gestion des paquets désordonnés.", "COMPLEX", "system-design", "fr"),

    _c("Projete um frontend de compilador com lexer, parser, AST, análise semântica e geração de IR. Inclua recuperação de erros e tabela de símbolos.", "COMPLEX", "system-design", "pt"),
    _c("Projete uma pilha TCP com controle de congestionamento, fluxo, retransmissão e gerenciamento de conexões. Inclua tratamento de pacotes fora de ordem.", "COMPLEX", "system-design", "pt"),

    _c("कंपाइलर फ्रंटएंड डिज़ाइन करें: लेक्सर, पार्सर, AST, शब्दार्थ विश्लेषण, IR जनरेशन। त्रुटि पुनर्प्राप्ति और प्रतीक तालिका शामिल करें।", "COMPLEX", "system-design", "hi"),

    _c("Derleyici ön ucu tasarlayın: lexer, parser, AST, anlamsal analiz, IR üretimi. Hata kurtarma ve sembol tablosu yönetimi dahil edin.", "COMPLEX", "system-design", "tr"),

    _c("Thiết kế frontend trình biên dịch: lexer, parser, AST, phân tích ngữ nghĩa, sinh IR. Bao gồm khôi phục lỗi và quản lý bảng ký hiệu.", "COMPLEX", "system-design", "vi"),

    _c("Zaprojektuj frontend kompilatora: lekser, parser, AST, analiza semantyczna, generowanie IR. Uwzględnij odzyskiwanie błędów i tabelę symboli.", "COMPLEX", "system-design", "pl"),

    # ── More COMPLEX to reach ~100 ──
    _c("Design a type checker for a language with Hindley-Milner, algebraic data types, and type classes. Include inference, error reporting, and module system.", "COMPLEX", "complex-code", "en"),
    _c("Build a distributed graph processing system with vertex-centric model, partitioning, and fault tolerance. Include checkpointing and recovery.", "COMPLEX", "system-design", "en"),
    _c("Design a real-time audio processing pipeline with sample rate conversion, filtering, and effects. Include latency optimization and buffer management.", "COMPLEX", "system-design", "en"),
    _c("Implement a multi-version concurrency control system with snapshot isolation, conflict detection, and garbage collection of old versions.", "COMPLEX", "complex-code", "en"),
    _c("Design a protocol buffer compiler with schema parsing, code generation for multiple languages, and plugin support. Include validation and documentation.", "COMPLEX", "system-design", "en"),
    _c("Build a distributed lock service with lease-based locking, fencing tokens, and failure detection. Include deadlock prevention.", "COMPLEX", "complex-code", "en"),
    _c("Design a mesh network routing protocol with link-state, path selection, and failure recovery. Include congestion awareness.", "COMPLEX", "system-design", "en"),
    _c("Implement a garbage collector with generational collection, concurrent marking, and compaction. Include write barriers and root scanning.", "COMPLEX", "complex-code", "en"),
    _c("Design a continuous integration system for compilers with incremental builds, test parallelization, and regression detection. Include caching.", "COMPLEX", "system-design", "en"),
    _c("Build a formal verification tool for concurrent programs with model checking and symbolic execution. Include counterexample generation.", "COMPLEX", "complex-code", "en"),

    # ── More non-English COMPLEX to reach ~100 ──
    _c("设计一个查询执行器，包括火山模型、嵌套循环/哈希/归并连接、聚合和排序。需要基于代价的计划选择。", "COMPLEX", "complex-code", "zh"),
    _c("实现一个 LSM 存储引擎，包括分层压缩、布隆过滤器和范围查询。需要优化写放大。", "COMPLEX", "complex-code", "zh"),
    _c("设计一个图分析平台，包括分布式存储、BFS/DFS/PageRank 和社区发现。需要增量更新。", "COMPLEX", "system-design", "zh"),
    _c("クエリ実行エンジンを設計してください。ボルケーノモデル、ネステッドループ/ハッシュ/マージジョイン、集約、ソートを含め、コストベースのプラン選択を含めてください。", "COMPLEX", "complex-code", "ja"),
    _c("LSMストレージエンジンを実装してください。レベル圧縮、ブルームフィルター、範囲クエリを含め、書き込み増幅の最適化を含めてください。", "COMPLEX", "complex-code", "ja"),
    _c("쿼리 실행기를 설계하세요. 볼케이노 모델, 중첩 루프/해시/머지 조인, 집계, 정렬, 비용 기반 계획 선택을 포함해야 합니다.", "COMPLEX", "complex-code", "ko"),
    _c("LSM 스토리지 엔진을 구현하세요. 레벨 압축, 블룸 필터, 범위 쿼리, 쓰기 증폭 최적화를 포함해야 합니다.", "COMPLEX", "complex-code", "ko"),
    _c("صمم محرك تنفيذ استعلامات مع نموذج البركان، حلقات متداخلة/هاش/دمج، تجميع وفرز. تضمن اختيار خطة قائم على التكلفة.", "COMPLEX", "complex-code", "ar"),
    _c("Спроектируй исполнитель запросов с вулканической моделью, вложенными циклами/хэш/слиянием, агрегацией и сортировкой. Включи выбор плана по стоимости.", "COMPLEX", "complex-code", "ru"),
    _c("Diseña un ejecutor de consultas con modelo volcánico, joins anidados/hash/merge, agregación y ordenación. Incluye selección de plan basada en coste.", "COMPLEX", "complex-code", "es"),
    _c("Entwerfe einen Query-Executor mit Volcano-Modell, Nested-Loop/Hash/Merge-Joins, Aggregation und Sortierung. Inkl. kostenbasierte Planauswahl.", "COMPLEX", "complex-code", "de"),
    _c("Conçois un exécuteur de requêtes avec modèle volcanique, jointures nested-loop/hash/merge, agrégation et tri. Inclure sélection de plan par coût.", "COMPLEX", "complex-code", "fr"),
    _c("Projete um executor de consultas com modelo vulcânico, joins nested-loop/hash/merge, agregação e ordenação. Inclua seleção de plano baseada em custo.", "COMPLEX", "complex-code", "pt"),
    _c("क्वेरी एक्ज़ीक्यूटर डिज़ाइन करें: वोल्कैनो मॉडल, नेस्टेड लूप/हैश/मर्ज जॉइन, एग्रीगेशन, सॉर्ट। लागत-आधारित योजना चयन शामिल करें।", "COMPLEX", "complex-code", "hi"),
    _c("Sorgu yürütücüsü tasarlayın: volkan modeli, iç içe döngü/hash/merge join, agregasyon, sıralama. Maliyet tabanlı plan seçimi dahil edin.", "COMPLEX", "complex-code", "tr"),
    _c("Thiết kế trình thực thi truy vấn với mô hình volcano, nested-loop/hash/merge join, aggregation, sort. Bao gồm lựa chọn kế hoạch dựa trên chi phí.", "COMPLEX", "complex-code", "vi"),
    _c("Zaprojektuj wykonawcę zapytań z modelem wulkanowym, złączeniami nested-loop/hash/merge, agregacją i sortowaniem. Uwzględnij wybór planu oparty na koszcie.", "COMPLEX", "complex-code", "pl"),
    _c("Design a distributed tracing system with span propagation, sampling strategies, and storage backends. Include correlation IDs and service dependency graphs.", "COMPLEX", "system-design", "en"),
]
...
REASONING_B11: list[dict] = [
    # ── Formal proofs ──
    _c("Prove that the set of regular languages is closed under intersection. Use the product construction.", "REASONING", "formal-proof", "en"),
    _c("Prove that the pumping lemma for regular languages holds. Show that if L is regular, then the lemma conditions are satisfied.", "REASONING", "formal-proof", "en"),
    _c("Prove that the set of context-free languages is closed under union but not under intersection.", "REASONING", "formal-proof", "en"),
    _c("Prove that every NFA can be converted to an equivalent DFA. Give the subset construction and prove correctness.", "REASONING", "formal-proof", "en"),
    _c("Prove that the language {a^n b^n c^n | n >= 0} is not context-free using the pumping lemma for CFLs.", "REASONING", "formal-proof", "en"),
    _c("Prove that the rationals are dense in the reals. Use the definition of density.", "REASONING", "formal-proof", "en"),
    _c("Prove that a tree with n vertices has exactly n-1 edges. Use induction.", "REASONING", "formal-proof", "en"),
    _c("Prove that every connected graph has a spanning tree. Construct it explicitly.", "REASONING", "formal-proof", "en"),
    _c("Prove that the maximum flow value equals the minimum cut capacity. Use the max-flow min-cut theorem.", "REASONING", "formal-proof", "en"),
    _c("Prove that a graph is bipartite if and only if it has no odd-length cycles. Show both directions.", "REASONING", "formal-proof", "en"),

    # ── Algorithm correctness proofs ──
    _c("Prove that Dijkstra's algorithm correctly computes shortest paths when all edge weights are non-negative. Use loop invariants.", "REASONING", "algorithm-proof", "en"),
    _c("Prove that Kruskal's algorithm produces a minimum spanning tree. Use the cut property.", "REASONING", "algorithm-proof", "en"),
    _c("Prove that the greedy activity selection algorithm yields an optimal solution. Use the exchange argument.", "REASONING", "algorithm-proof", "en"),
    _c("Prove that Bellman-Ford correctly finds shortest paths when no negative cycles exist. Use induction on path length.", "REASONING", "algorithm-proof", "en"),
    _c("Prove that the Ford-Fulkerson method finds a maximum flow when capacities are integers. Show that each augmentation increases flow by at least 1.", "REASONING", "algorithm-proof", "en"),
    _c("Prove that binary search runs in O(log n) time and finds the element if present. Use the loop invariant.", "REASONING", "algorithm-proof", "en"),
    _c("Prove that the greedy Huffman algorithm produces an optimal prefix code. Use the lemma about optimal codes for sorted frequencies.", "REASONING", "algorithm-proof", "en"),
    _c("Prove that merge sort is correct and runs in O(n log n). Use the recursion tree and master theorem.", "REASONING", "algorithm-proof", "en"),
    _c("Prove that the two-pointer technique correctly finds a pair with given sum in a sorted array. Show termination.", "REASONING", "algorithm-proof", "en"),
    _c("Prove that Prim's algorithm produces a minimum spanning tree. Use the cut property.", "REASONING", "algorithm-proof", "en"),

    # ── Math derivations ──
    _c("Derive the closed-form solution for T(n) = 2T(n/2) + n using the master theorem. Show each case.", "REASONING", "math-derivation", "en"),
    _c("Derive the expected number of comparisons in randomized quicksort. Use linearity of expectation.", "REASONING", "math-derivation", "en"),
    _c("Derive the optimal policy for a simple MDP using value iteration. Prove convergence.", "REASONING", "math-derivation", "en"),
    _c("Derive the Cramer-Rao lower bound for an unbiased estimator. Use the Cauchy-Schwarz inequality.", "REASONING", "math-derivation", "en"),
    _c("Derive the bias-variance decomposition for squared error loss. Show that E[(Y - f)^2] = Bias^2 + Var + Noise.", "REASONING", "math-derivation", "en"),
    _c("Derive the formula for the sum of the first n squares: 1² + 2² + ... + n² = n(n+1)(2n+1)/6. Use induction.", "REASONING", "math-derivation", "en"),
    _c("Derive the optimal stopping rule for the secretary problem. Prove that 1/e is the threshold.", "REASONING", "math-derivation", "en"),
    _c("Derive the recurrence for the number of binary search trees with n nodes. Solve it to get Catalan numbers.", "REASONING", "math-derivation", "en"),
    _c("Derive the Fisher information for the normal distribution. Use the definition I(θ) = E[(∂/∂θ log f(X;θ))²].", "REASONING", "math-derivation", "en"),
    _c("Derive the optimal k for k-means when the true number of clusters is known. Use the elbow method justification.", "REASONING", "math-derivation", "en"),
    ...
    _c("Prove by contradiction that the halting problem is undecidable. Use diagonalization.", "REASONING", "formal-proof", "en"),
    _c("Prove by induction that a full binary tree with n internal nodes has n+1 leaves.", "REASONING", "formal-proof", "en"),
    _c("Prove by contradiction that there is no largest prime number.", "REASONING", "formal-proof", "en"),
    _c("Prove by induction that F(n) * F(n+1) = F(1)² + F(2)² + ... + F(n)² for Fibonacci numbers.", "REASONING", "formal-proof", "en"),
    ...
    _c("Show that the greedy coin change algorithm is optimal for coin systems {1, 5, 10, 25}. Prove it.", "REASONING", "algorithm-proof", "en"),
    _c("Demonstrate that the perceptron algorithm converges when the data is linearly separable. Provide a proof.", "REASONING", "algorithm-proof", "en"),
    _c("Establish that the set of decidable languages is closed under complement. Give a formal proof.", "REASONING", "formal-proof", "en"),
    _c("Verify that the greedy set cover algorithm has approximation ratio H(n). Prove the bound.", "REASONING", "algorithm-proof", "en"),

    # ── Game theory / logic ──
    _c("Prove that every finite two-player zero-sum game has a Nash equilibrium in mixed strategies. Use the minimax theorem.", "REASONING", "game-theory", "en"),
    _c("Prove that in the iterated prisoner's dilemma, tit-for-tat is a Nash equilibrium when the discount factor is high enough.", "REASONING", "game-theory", "en"),
    _c("Prove that (A → B) → ((B → C) → (A → C)) is a tautology using natural deduction.", "REASONING", "formal-logic", "en"),
    _c("Prove that resolution is refutation-complete for propositional logic. Show that any unsatisfiable formula has a resolution refutation.", "REASONING", "formal-logic", "en"),
    _c("Prove that 2-SAT can be solved in linear time by reduction to strongly connected components.", "REASONING", "formal-logic", "en"),
    ...
    _c("正規言語が交叉の下で閉じていることを証明してください。積構成を用いてください。", "REASONING", "formal-proof", "ja"),
    _c("Dijkstraのアルゴリズムが非負の重みで最短経路を正しく計算することを証明してください。ループ不変式を用いてください。", "REASONING", "algorithm-proof", "ja"),
    _c("マスター定理を用いてT(n)=2T(n/2)+nの閉形式を導出してください。各ケースを示してください。", "REASONING", "math-derivation", "ja"),
    ...
    _c("Dijkstra 알고리즘이 음이 아닌 가중치에서 최단 경로를 올바르게 계산함을 증명하세요. 루프 불변식을 사용하세요.", "REASONING", "algorithm-proof", "ko"),
    _c("마스터 정리를 사용해 T(n)=2T(n/2)+n의 닫힌 형태를 유도하세요. 각 경우를 보여주세요.", "REASONING", "math-derivation", "ko"),
    ...
    _c("أثبت أن خوارزمية Dijkstra تحسب المسارات الأقصر بشكل صحيح عندما تكون الأوزان غير سالبة. استخدم ثوابت الحلقة.", "REASONING", "algorithm-proof", "ar"),

    _c("Докажи, что регулярные языки замкнуты относительно пересечения. Используй произведение автоматов.", "REASONING", "formal-proof", "ru"),
    _c("Докажи, что алгоритм Дейкстры правильно вычисляет кратчайшие пути при неотрицательных весах. Используй инвариант цикла.", "REASONING", "algorithm-proof", "ru"),

    _c("Demuestra que los lenguajes regulares son cerrados bajo intersección. Usa la construcción del producto.", "REASONING", "formal-proof", "es"),
    _c("Demuestra que el algoritmo de Dijkstra calcula correctamente los caminos más cortos con pesos no negativos. Usa invariantes de bucle.", "REASONING", "algorithm-proof", "es"),

    _c("Beweise, dass reguläre Sprachen unter Schnitt abgeschlossen sind. Verwende die Produktkonstruktion.", "REASONING", "formal-proof", "de"),
    _c("Beweise, dass Dijkstras Algorithmus bei nichtnegativen Gewichten korrekt kürzeste Wege berechnet. Verwende Schleifeninvarianten.", "REASONING", "algorithm-proof", "de"),

    _c("Démontre que les langages réguliers sont clos par intersection. Utilise la construction produit.", "REASONING", "formal-proof", "fr"),
    _c("Démontre que l'algorithme de Dijkstra calcule correctement les plus courts chemins avec des poids non négatifs. Utilise les invariants de boucle.", "REASONING", "algorithm-proof", "fr"),

    _c("Prove que as linguagens regulares são fechadas sob interseção. Use a construção do produto.", "REASONING", "formal-proof", "pt"),
    _c("Prove que o algoritmo de Dijkstra calcula corretamente os caminhos mais curtos com pesos não negativos. Use invariantes de loop.", "REASONING", "algorithm-proof", "pt"),

    _c("सिद्ध करें कि नियमित भाषाएँ प्रतिच्छेदन के अंतर्गत संवृत हैं। गुणन निर्माण का उपयोग करें।", "REASONING", "formal-proof", "hi"),

    _c("Düzenli dillerin kesişim altında kapalı olduğunu kanıtlayın. Çarpım yapısını kullanın.", "REASONING", "formal-proof", "tr"),

    _c("Chứng minh rằng các ngôn ngữ chính quy đóng dưới phép giao. Sử dụng cấu trúc tích.", "REASONING", "formal-proof", "vi"),

    _c("Udowodnij, że języki regularne są zamknięte ze względu na przecięcie. Użyj konstrukcji iloczynu.", "REASONING", "formal-proof", "pl"),

    # ── More REASONING to reach ~80 ──
    _c("Prove that the greedy interval scheduling algorithm is optimal. Use the exchange argument.", "REASONING", "algorithm-proof", "en"),
    _c("Prove that the set of recursively enumerable languages is closed under union. Give a constructive proof.", "REASONING", "formal-proof", "en"),
    _c("Prove that a graph has an Eulerian circuit if and only if every vertex has even degree. Show both directions.", "REASONING", "formal-proof", "en"),
]
